...
class Agent:
    ...
    def build_model(self):
        ...
        # fc = TimeDistributed(Dense(24, activation='relu'))(input)

        net = LSTM(16, activation='relu', return_sequences=True)(input)
        net = LSTM(24, activation='relu')(net)
...
if __name__ == "__main__":
    ...
    for e in range(EPISODES):
        ...
        while not done:
            ...
            # 현재 상태로 행동을 선택
            st = np.reshape(state, [1, 1, state_size])
            ...
            if done:
                ...
                xs = np.vstack(xs)
                xs = np.reshape(xs, (len(xs), 1, state_size))
                ys = np.vstack(ys)
                ...
                pylab.savefig("./save_graph/cartpole_a2c_lstm.png")
                ...
                if np.mean(scores[-min(30, len(scores)):]) > 490:
                    agent.actor.save_weights("./save_model/cartpole_a2c_lstm_actor.h5")
                    agent.critic.save_weights("./save_model/cartpole_a2c_lstm_critic.h5")
